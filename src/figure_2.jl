...
function conditional_distance_distribution(s::Sequences, truep::Vector{Int}, genome_path::String, params::Vector{Tuple{Int,Int}})
    ...
    #reverse_complement!(s)
    #fasta2_path = joinpath(@__DIR__, "test_linear_2.fasta.gz")
    #write(fasta2_path, s)
    mm_pairss = []
    lens = Int[]
...
function probabilities_distances(genome_path::String, unique_set::Set{Int}, params::Vector{Tuple{Int,Int}};
    ...
    rates = [
        begin
            ...
            count_similar ./ len#(length(correct))
...
function plot_data(lens::Vector{Int}, lmin::Int, lmax::Int, lmaxadapter::Int, nerrmax::Int, nseqs::Int,
        seedlens::Vector{Int}, scores::Vector{Int}, maxd::Int, params::Vector{Pair{Tuple{Int,Int}, String}})
    genome_path = joinpath(@__DIR__, "rand_genome.fna")
    ...
    #ax_zoom = Axis(ga[1,2], xlabel="FPR", ylabel="TPR")
    mkpath(joinpath(@__DIR__, "csv"))
    us = unique_set(g)
    ...
    for (j,nerr) in enumerate(0:nerrmax), (i,l) in enumerate(lens)
        ...
        label = "$l | $nerr"
        fname_tpr = joinpath(@__DIR__, "csv", name * "_tpr.csv")
        fname_fpr = joinpath(@__DIR__, "csv", name * "_fpr.csv")
        fname_found = joinpath(@__DIR__, "csv", name * "_unique.csv")
        tpr, fpr, _ = bwa_mem_tpr_fpr(fname_tpr, fname_fpr, fname_found, s, truep, g, seedlens, scores)
        #noisex = (rand()-0.5)*0.001
        #noisey = (rand()-0.5)*0.001
        ...
        #scatter!(ax_zoom, [first(p) for p in sorted_points], [last(p) for p in sorted_points], label=name, mode="markers", alpha=0.6)
    ...
    scatter!(ax, [first(p) for p in sorted_points], [last(p) for p in sorted_points], label=label, mode="markers", alpha=0.6)
    #scatter!(ax_zoom, [first(p) for p in sorted_points], [last(p) for p in sorted_points], label=name, mode="markers", alpha=0.6)
    #xlims!(ax_zoom, (-0.001,0.002))
    #ylims!(ax_zoom, (0.987,0.993))
    ...
    #ax_ints_count = Axis(gb[1,2], ylabel="interacting pairs", xlabel="reads per pair", title="RIL-seq chimeras", yscale=log10, xscale=log10)
    ...
    for (((se, ms), label), color) in zip(params, colors)
        ...
        for (i, pcut) in enumerate(pcuts)
            ...
            #println(pcut)
            #println(median(interact.edges[pindex, :nb_ints]))
            #println(median(interact.edges[1:sum(pindex), :nb_ints]))
        end
        ...
        #randsubset = rand(1:nrow(interact.edges), sum(interact.edges.bp_fdr .<= 0.05))
        #corr = [corspearman(interact.edges[randsubset, p1], interact.edges[randsubset, p2]) for (p1, p2) in combinations(interact.replicate_ids, 2)]
        #corr_mean[length(pcuts)+2] = mean(corr)
        #corr_sd[length(pcuts)+2] = std(corr)
        #push!(corr_means, corr_mean)
        #push!(corr_sds, corr_sd)
        #scatter!(ax_cor, 1:(length(pcuts)+1), corr_top_mean, color=:black, label=l)

        #count_interactions = Dict{Int, Int}()
        #for row in eachrow(interact.edges)
        #    row.nb_ints in keys(count_interactions) ? count_interactions[row.nb_ints] += 1 : count_interactions[row.nb_ints] = 1
        #end
        #sorted_count_interactions = sort(collect(count_interactions), by=x->x[1])
        #sorted_count_interactions = sorted_count_interactions[1:findfirst(x->x[1] > 100, sorted_count_interactions)]
        #scatter!(ax_ints_count, first.(sorted_count_interactions), last.(sorted_count_interactions), color=color)

        scatter!(ax_ints_count, 1:length(pcuts), subcounts, color=color)
    end
    #barplot!(ax_cor, 1:(length(pcuts)+1), repeat([0.6], length(pcuts)+1), width=0.4, color=RGBAf(0.6, 0.6, 0.6, 0.4))
    #scatter!(ax_cor, 1:(length(pcuts)+1), repeat([0.6], length(pcuts)+1), width=0.4, color=RGBAf(0.6, 0.6, 0.6, 0.4))
    #ylims!(ax_count, (0, (max_count+0.1*max_count)/10^4))
    ...
    compare_df = DataFrame(CSV.File(joinpath(@__DIR__, "csv_resampled/", "stripped_interactions_hfq_hcd.csv")))[1:200, :]
    ...
    for (i, (((se, ms), label), color)) in enumerate(zip(params, colors))
        resampled_df = DataFrame(CSV.File(joinpath(@__DIR__, "csv_resampled/", "interactions_resampled_$(se)_$(ms).csv")))
        ...
        nfound = sum(k in keys(found_dict) for k in keys(pair_dict))
        mean_found_ratio = mean(v/pair_dict[k] for (k,v) in found_dict if v != 0.0)
        sd_found_ratio = std(v/pair_dict[k] for (k,v) in found_dict if v != 0.0)
        ...
        #barplot!(ax_ratio, [i], [mean_found_ratio], color=color)
        #errorbars!(ax_ratio, [i], [mean_found_ratio], [sd_found_ratio], whiskerwidth=5)
    end
    #probs_file = joinpath(@__DIR__, "csv", "probs.csv")
    #probss = if isfile(probs_file)
    #    df = DataFrame(CSV.File(probs_file))
    #    [vec(df[!, co]) for co in last.(params)]
    #else
    #    p = probabilities_distances(genome_path, us, first.(params); minlen=lmin, maxlen=lmax, maxrandlen=lmaxadapter, nseqs=10000000, maxerr=nerrmax, maxd=maxd, repeateach=10)
    #    CSV.write(probs_file, DataFrame(Dict(last.(params)[i]=>pbs for (i,pbs) in enumerate(p))))
    #    p
    #end
    #ax1 = Axis(gb[1,1], title="close misalignments", xlabel="distance", ylabel="average close misalignments")

    #for (probs, ((se, ms), label), color) in zip(probss, params, colors)
    #    scatter!(ax1, 100:100:maxd, probs[1:length(100:100:maxd)], label=label, color=color)
    #end
    #Legend(gb[1,5], ax1)
    #axislegend(ax1, position=:rb)

    save(joinpath(@__DIR__, "figure_align_mem.svg"), fig)

    save(joinpath(@__DIR__, "figure_align_mem_si.svg"), fig_si)

end

plot_data([15,25,40], 15, 40, 0, 1, 100000, [12,14,15,16,18], [14,16,17,18,20], 2000,
